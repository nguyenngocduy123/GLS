...
export class PlannerDataImportComponent implements OnInit {
    ...
    load() {
        ...
        forkJoin([
            ...
            this._vehicleValidator = new VehicleValidator(vehicles, driverUsers, usergroups.map((u) => u.usergroup), this._vehicleTypeValidator);
            this._orderValidator = new OrderValidator(this._vehicleValidator, this._vehicleTypeValidator);
    ...
    validateAllTables() {
        ...
        this.tableList.forEach((table) => {
            ...
            if (tableColumn) {
                ...
                switch (t) {
                    case 'Order':
                        this._validateOrder(data);
                        break;
                    case 'DeliveryItem':
                        this._validateDeliveryItem(data);
                        break;
                    case 'Vehicle':
    ...
    private _validateOrder(data: any[]) {
        this._checkPostalCode(data, 'Postal');
        this._orderValidator.validate(data);
        this._checkOrderIdExist(data);
    }

    private _checkOrderIdExist(data: any[]) {
        const where = { Id: this._orderValidator.getOrderIds() };
        this._plannerRest.queryDeliveryMasters(undefined, undefined, where, true).subscribe((res) => {
            const existingIds = res.map((o) => o.Id);
            data.filter((r) => existingIds.includes(r.Id)).forEach((r) => ValidatorUtils.setAlreadyExistError(r, 'OrderId'));
            this.checkDataValid();
        }, (err) => this._dialog.errorResponse(err));
    }

    private _checkPostalCode(data: any[], prop: string) {
