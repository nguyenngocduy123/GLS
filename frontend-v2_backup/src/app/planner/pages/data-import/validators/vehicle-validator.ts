...
export class VehicleValidator {
    ...
    private async _validateExistingId(data: any[]) {
        const vehicleIds: string[] = data.map((r) => r.Id);
        await this._plannerRest.checkForDuplicates(vehicleIds, this._tableName).subscribe((duplicates) => {
            duplicates.map((id) => ValidatorUtils.setAlreadyExistError(data[vehicleIds.indexOf(id)], 'Id'));
        });
