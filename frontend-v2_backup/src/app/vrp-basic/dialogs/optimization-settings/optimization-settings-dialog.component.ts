...
import { ORDER_SELECTION_CONFIG, PROBLEMDETAIL_TABLE_CONFIG, OPTIMIZATION_CONFIG, OPTIMIZATION_ENGINES } from '@app/vrp-basic/vrp-basic.config';
...
export class VrpOptimizationSettingsDialogComponent implements OnInit {
    ...
    close() {
        ...
        if (this._preserveInitialRoutes) {// if initial solution is preserved
            if (options.engine === OPTIMIZATION_ENGINES.SIWEI_ENGINE) {
    ...
    onEngineChange(event) {
        if (event.value === OPTIMIZATION_ENGINES.SIWEI_ENGINE) {
            this._updateReadOnlyElements(true);
        } else {
            this._updateReadOnlyElements(false);
        }
    }

    private _updateReadOnlyElements(readOnly: boolean) {
        this.elements.forEach((ele) => {
            ele.forEach((sub) => {
                if (_startsWith(sub.label, '*')) {
                    sub.readonly = readOnly;
                    this.form[sub.name] = (readOnly) ? undefined : sub.default;
                }
            });
        });
    }
